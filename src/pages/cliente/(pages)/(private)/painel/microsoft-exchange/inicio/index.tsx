...
import { useClientMicrosoftExchange } from "@/repositories/clientMicrosoftExchangeRepository";
import { IServicosMicrosoftExchange } from "@/repositories/types";
import MicrosoftExchangeCard from "@/pages/cliente/components/MicrosoftExchange/view";

export default function HomeMicrosoftExchangeView() {

    const { planoMicrosoftExchange, actions: { setClienMicrosoftExchanges } } = useClientStore()
    const { getClientID } = useUtils()
    const { data, isLoading, isError, isSuccess } = useClientMicrosoftExchange(getClientID())
    const [search, setSearch] = useState('')

    useEffect(() => {
        setClienMicrosoftExchanges(data as IServicosMicrosoftExchange[])
        console.log("painel microsoft exchange", planoMicrosoftExchange)
    }, [data, planoMicrosoftExchange])

    const filteredPlans = useMemo(() => {
        if (!planoMicrosoftExchange || planoMicrosoftExchange.length === 0) {
            return [];
        }
        return planoMicrosoftExchange.filter((plan) =>
            plan.PlanoMicrosoftExchange.titulo && plan.PlanoMicrosoftExchange.titulo .toLowerCase().includes(search.toLowerCase())
        );
    }, [search, planoMicrosoftExchange]);
            ...
            <div className="w-full">
                <div className="flex items-center justify-between w-full h-[80px] my-[15px]">
                    <div className="flex flex-col items-start justify-center">
                        <h1 className="text-[#616161] font-[400] text-[2rem] font-[Rubik]" style={{ lineHeight: 1 }}>Serviços Microsoft Exchange</h1>
                ...
                <div className='w-full flex gap-2 justify-center font-[Rubik]'  >
                    <div className="flex gap-4 items-center justify-start w-full">
                        ...
                        <Link to={'/email-office-365'} className="flex items-center justify-center gap-2 h-[38px] text-[0.8rem] px-3 rounded-[8px] text-cyan-800 font-[Rubik] no-underline border-[1px] border-solid border-[#305b83]" >Contratar produto <GoPlus size={18} color="rgb(21 94 117)" /></Link>
                    </div>
                ...
                <div className="w-full bg-transparent overflow-y-scroll flex-col gap-y-3 mt-[15px] rounded-[12px] flex" style={{ height: 'calc(100vh - 240px)' }}>
                    ...
                    }
                    {
                        (isSuccess && filteredPlans) && filteredPlans.map((item, _index) => (
                            <MicrosoftExchangeCard key={_index} service={item} />
                        ))
                    //    planoMicrosoftExchange && planoMicrosoftExchange.map((item, _index) => (
                    //         <MicrosoftExchangeCard key={_index} service={item} />
                    //     ))
                    }
                    {
                    ...
                    {
                        ((planoMicrosoftExchange && planoMicrosoftExchange.length === 0) && isSuccess) && (
                            <div className="w-full h-full flex flex-col gap-3 items-center justify-center">
                                <img src={svg} alt="wihout services image" className="w-[150px] h-[150px]" />
