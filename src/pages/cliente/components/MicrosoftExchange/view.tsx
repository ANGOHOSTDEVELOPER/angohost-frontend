...
interface HostCardModel {
    service: IServicosMicrosoftExchange
}
...
export default function MicrosoftExchangeCard({ service }: HostCardModel) {

    const { formatDate, formatMoney } = useUtils()
    const { actions: {
        setCurrentMicrosoftExchangeService
    } } = useClientStore()
    ...
    return (
        <div className="w-full h-[80px] bg-[#ffffff] rounded-[12px] flex items-center justify-between p-4" style={{ boxShadow: "0 2px 2px rgba(0, 0, 0, 0.014)" }}>
            ...
            <div className="h-max flex-1 ml-7">
                <h1 className="font-[Rubik] opacity-80 text-[1rem] font-semibold" style={{ lineHeight: '15px' }}>{service.PlanoMicrosoftExchange.titulo}</h1>
            ...
            <div className="flex-1 flex flex-col items-center justify-center">
                ...
                <p className="font-[Rubik] text-[0.8rem] opacity-50 mt-1">{formatMoney(service.PlanoMicrosoftExchange.preco * service.ciclo.multiplicador)}</p>
            </div>
            <div className="h-full flex items-center justify-end flex-1">
                {service.status === "ATIVO" && (
                    <button
                        onClick={() => {
                            setCurrentMicrosoftExchangeService(service);
