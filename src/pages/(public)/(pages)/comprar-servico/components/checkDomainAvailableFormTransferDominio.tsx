...
import { DomainTransferirStatusModal } from "@/pages/(public)/components/domainStatusModal"
...
export default function CheckDomainAvailableFormTransfer() {
    ...
    return (
        <form onSubmit={handleSubmit(verifyDomain)} className="search-bar-dominio-center-flex flex-col md:flex-row gap-y-2 mt-2"  >
            ...
            <button style={{ textDecoration: "none" }} className=" md:rounded-r-[8px] text-white font-medium w-full md:w-[150px] h-[67px] flex items-center justify-center bg-[#0161a0]">
                {isLoading ? <LoaderComponent color="#fff" /> : "Transferir"}
            </button>
            <RejectModal openedReject={opendedReject} setOpenedReject={setOpendedReject} />
            <DomainTransferirStatusModal opened={opened} setOpened={setOpened} status={currentDomainAvailable} currentDomain={currentDomain} price={(currentDomain.split('.')[0].length === 3  && (currentDomainExtension.tipo === '.ao' || currentDomainExtension.tipo === '.co.ao')) ? 300000 : currentDomainExtension.preco}/>
        </form>
    )
}