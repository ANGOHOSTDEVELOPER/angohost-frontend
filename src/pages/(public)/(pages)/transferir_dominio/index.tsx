...
import CheckDomainAvailableFormTransfer from "../comprar-servico/components/checkDomainAvailableFormTransferDominio";

export const PagesTransferenciaDeDominios = () => {
    const [m, setM] = useState(false);
    const { formatMoney } = useUtils();

    const {
        domainExtensions,
        actions: { getDomainExtensions },
    } = usePayStore();
    ...
    return (
        <>
            <Helmet>
                <title>Transfira os dominios com a ANGOHOST</title>
            </Helmet>

            <NavbarFull logo={LOGOBRANCO} color="#ffff" />
            <div
                className="container-dominio  "
                style={{
                    background:
                        "linear-gradient(-90deg, rgba(43, 6, 67, 0.85), rgb(120, 3, 121), rgb(2, 18, 31))",
                }}
            >
                <header className="header-dominio">
                    <div className="header-dominio-start">
                        <h1>Transferência de Domínio na AngoHost: Rápido, Seguro e Simples!</h1>
                ...
                <div className="search-bar-dominio-center">
                    <CheckDomainAvailableFormTransfer />
                </div>
                    ...
                    <section className="domain-options">
                        ...
                        {domainExtensions.slice(0, 4).map((domain, _index) => (
                            <div className="domain-option" key={_index}>
                                <span>{domain.tipo}</span>
                                <span className="price-domino">
                                    {formatMoney(domain.preco)}
                                </span>
                            </div>
                        ))}
                    </section>
                ...
                <div className="container-afinal-flex">
                    <div className="container-afinal-left">
                        <div className="container-afinal-left-info">
                            ...
                            <p>
                                Assim como no mundo offline, ter um bom endereço
                                é muito importante para os negócios. Um bom nome
                                de domínio garante que seus clientes encontrem
                                sua empresa com mais facilidade na internet.
                                Veja outras vantagens de registrar um domínio:
                            </p>
                            <div className="container-afinal-left-info-list">
                                <p>
                                    <RiStarSFill color="#28b16c" />
                                    Aumenta a credibilidade do nome da sua
                                    empresa
                                </p>
                                <p>
                                    <RiStarSFill color="#28b16c" />
                                    Unifica os pontos de contato da sua marca
                                </p>
                                <p>
                                    <RiStarSFill color="#28b16c" />
                                    Sua empresa, loja virtual ou blog online 24h
                                    por dia
                                </p>
                            </div>
                        </div>
